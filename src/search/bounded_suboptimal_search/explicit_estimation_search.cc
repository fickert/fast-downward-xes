#include "explicit_estimation_search.h"
...
ExplicitEstimationSearch<tie_breaking>::ExplicitEstimationSearch(const Options &opts)
	: EagerSuboptimalSearch<3>(opts),
	  suboptimality_factor(opts.get<double>("suboptimality_factor")),
	  use_node_selection_fix(opts.get<bool>("use_node_selection_fix")),
...
template <bool tie_breaking>
auto ExplicitEstimationSearch<tie_breaking>::open_list_top() -> std::optional<SearchNode> {
...
template <bool tie_breaking>
void ExplicitEstimationSearch<tie_breaking>::open_list_pop() {
...
template <bool tie_breaking>
void ExplicitEstimationSearch<tie_breaking>::open_list_push(EvaluationContext &eval_context, const EvaluatorValues &evaluator_values, StateID state_id,
...
template <bool tie_breaking>
auto ExplicitEstimationSearch<tie_breaking>::get_debiased_bound() -> std::optional<double> {
	auto top_node = open_list_top();
	if (!top_node)
		return {};
	auto eval_context = EvaluationContext(top_node->get_state(), top_node->get_g(), false, &statistics, false);
	const auto best_f_hat = f_hat_evaluator->compute_result(eval_context);
	return suboptimality_factor * best_f_hat;
}

template <bool tie_breaking>
void ExplicitEstimationSearch<tie_breaking>::update_focal() {
	const auto current_debiased_bound = get_debiased_bound();
	if (!current_debiased_bound)
		return;

	if (*current_debiased_bound > debiased_bound) {
		// the bound increased --> we need to copy some nodes into the focal list
		const auto lower = open_list.upper_bound(debiased_bound);
		const auto upper = open_list.upper_bound(*current_debiased_bound);
		...
		// the bound decreased --> we need to remove some nodes from the focal list
		const auto lower = open_list.upper_bound(*current_debiased_bound);
		const auto upper = open_list.upper_bound(debiased_bound);
		for (auto it = lower; it != upper; ++it) {
			for (auto &open_list_element : it->second) {
				auto &focal_handle = std::get<std::optional<FocalListHandleType>>(open_list_element);
				const auto state_id = std::get<StateID>(open_list_element);
				const auto state = state_registry.lookup_state(state_id);
				const auto node = search_space.get_node(state);
				auto eval_context = EvaluationContext(state, node.get_g(), false, &statistics);
				if (!node.is_closed() && f_hat_evaluator->compute_result(eval_context) == it->first) {
					// the node should have a (valid!) handle
					assert(focal_handle);
					focal_list.erase(*focal_handle);
				}
				focal_handle.reset();
			}
		}
	}
	debiased_bound = *current_debiased_bound;
}

template <bool tie_breaking>
auto ExplicitEstimationSearch<tie_breaking>::get_cost_estimate(EvaluationContext &eval_context) const -> double {
	if (use_node_selection_fix) {
		assert(!eval_context.is_evaluator_value_infinite(heuristic.get()));
		return eval_context.get_g_value() + eval_context.get_evaluator_value(heuristic.get());
	} else {
		return f_hat_evaluator->compute_result(eval_context);
	}
}

template <bool tie_breaking>
auto ExplicitEstimationSearch<tie_breaking>::fetch_next_node() -> std::optional<SearchNode> {
	...
	const auto bound = suboptimality_factor * (best_f->get_g() + best_f_eval_context.get_evaluator_value(heuristic.get()));
	...
	if (best_d_hat) {
		...
		if (get_cost_estimate(best_d_hat_eval_context) <= bound) {
			focal_list.pop();
	...
	auto best_f_hat = open_list_top();
	assert(best_f_hat); // the open list should only be empty if the cleanup list is empty, and we catch that case above
	auto best_f_hat_eval_context = EvaluationContext(best_f_hat->get_state(), best_f_hat->get_g(), false, &statistics, false);
	if (get_cost_estimate(best_f_hat_eval_context) <= bound) {
		close_node(best_f_hat);
		return best_f_hat;
	} else {
		cleanup_list.pop();
		close_node(best_f);
...
template <bool tie_breaking>
void ExplicitEstimationSearch<tie_breaking>::insert(EvaluationContext &eval_context, const EvaluatorValues &evaluator_values, StateID state_id, bool preferred) {
	auto focal_handle = std::optional<typename decltype(focal_list)::handle_type>();
	if (evaluator_values[F_HAT_INDEX] <= debiased_bound) {
		if constexpr (tie_breaking) {
			assert(!eval_context.is_evaluator_value_infinite(heuristic.get()));
			const auto h = eval_context.get_evaluator_value(heuristic.get());
...
template <bool tie_breaking>
void ExplicitEstimationSearch<tie_breaking>::reward_progress() {
	// preferred operators are not (yet?) implemented for EES --> nothing to do here
}

static auto _parse_ees(OptionParser &parser) -> std::shared_ptr<SearchEngine> {
	...
	parser.add_option<bool>("use_node_selection_fix", "Use f instead of f-hat in the node selection function.", "false");
	...
	return opts.get<bool>("enable_tie_breaking") ? std::static_pointer_cast<SearchEngine>(std::make_shared<ExplicitEstimationSearch<true>>(opts))
	                                             : std::static_pointer_cast<SearchEngine>(std::make_shared<ExplicitEstimationSearch<false>>(opts));
}

static options::Plugin<SearchEngine> _plugin_bees("ees", _parse_ees);

template class ExplicitEstimationSearch<true>;
template class ExplicitEstimationSearch<false>;
} // namespace bounded_suboptimal_search
