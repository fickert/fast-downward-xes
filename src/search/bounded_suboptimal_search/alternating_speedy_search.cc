#include "alternating_speedy_search.h"
...
AlternatingSpeedySearch::AlternatingSpeedySearch(const Options &opts)
	: SearchEngine(opts),
	...
	if (opts.contains("preferred") && !opts.get_list<std::shared_ptr<Evaluator>>("preferred").empty()) {
		std::cerr << "Alternating speedy search currently does not support preferred operators, exiting." << std::endl;
...
auto AlternatingSpeedySearch::fetch_next_node() -> std::optional<SearchNode> {
	...
	if (current_f_min > f_min) {
		// f_min increased --> fix d/f-hat lists by copying all nodes that were previously
		...
		for (auto it = lower; it != upper; ++it) {
			for (auto [_, state_id] : it->second) {
				...
				const auto d = static_cast<double>(eval_context.get_evaluator_value(distance.get()));
				d_list.emplace({d}, std::move(state_id), false);
	...
	while (true) {
		...
		if (statistics.get_expanded() % 3 == 2) {
			auto &state_ids = std::begin(f_list)->second;
			...
			auto &selected_open_list = statistics.get_expanded() % 3 == 0 ? d_list : f_hat_list;
			assert(!selected_open_list.empty());
			id = selected_open_list.top();
			selected_open_list.pop();
		}
...
void AlternatingSpeedySearch::insert(EvaluationContext &eval_context) {
	...
	if (f <= suboptimality_factor * f_min) {
		const auto d = static_cast<double>(eval_context.get_evaluator_value(distance.get()));
		d_list.push({d}, state_id, eval_context.is_preferred());
		const auto f_hat = f_hat_evaluator->compute_result(eval_context);
		f_hat_list.push({f_hat}, state_id, eval_context.is_preferred());
	}
}

void AlternatingSpeedySearch::initialize() {
...
void AlternatingSpeedySearch::print_statistics() const {
	statistics.print_detailed_statistics();
	search_space.print_statistics();
}

auto AlternatingSpeedySearch::step() -> SearchStatus {
...
void AlternatingSpeedySearch::reward_progress() {
	// preferred operators are not (yet?) implemented for DPS --> nothing to do here
}

static auto _parse_alternating_speedy(OptionParser &parser) -> std::shared_ptr<SearchEngine> {
	...
	return std::make_shared<AlternatingSpeedySearch>(opts);
}

static options::Plugin<SearchEngine> _plugin_dps("alt_d", _parse_alternating_speedy);
} // namespace bounded_suboptimal_search
