#ifndef BOUNDED_SUBOPTIMAL_SEARCH_EXPLICIT_ESTIMATION_SEARCH_H
#define BOUNDED_SUBOPTIMAL_SEARCH_EXPLICIT_ESTIMATION_SEARCH_H
...
class ExplicitEstimationSearch : public suboptimal_search::EagerSuboptimalSearch<3> {
	...
	const bool use_node_selection_fix;
	...
	double debiased_bound;

	auto get_debiased_bound() -> std::optional<double>;
	void update_focal();
...
protected:
	...
	// get cost estimate in node selection
	auto get_cost_estimate(EvaluationContext &eval_context) const -> double;
	...
	using FocalListValueType = std::conditional_t<tie_breaking, std::pair<std::tuple<double, double, int>, StateID>, std::pair<double, StateID>>;
...
public:
	explicit ExplicitEstimationSearch(const options::Options &opts);
	virtual ~ExplicitEstimationSearch() = default;
};
} // namespace bounded_suboptimal_search
